...
#define in64 (long long)(unsigned short)
...
// Optimization for inlining Interlocked functions
...
#ifdef _M_IX86
#define InterlockedExchangePointer(Target, Value) (PVOID)_InterlockedExchange((PLONG)(Target), (LONG)(Value))
#else
...
// struct VideoInfo
  ...
  int     (VideoInfo::*FramesFromAudioSamples)(long long samples) const;
  long long (VideoInfo::*AudioSamplesFromBytes)(long long bytes) const;
  long long (VideoInfo::*BytesFromAudioSamples)(long long samples) const;
...
*/
  ...
  long long num_audio_samples;      // changed as of 2.5
...
//bool IsYUV9()  const;
  ...
  long long AudioSamplesFromFrames(int frames) const AVS_BakedCode( return AVS_LinkCall(AudioSamplesFromFrames)(frames) )
  int FramesFromAudioSamples(long long samples) const AVS_BakedCode( return AVS_LinkCall(FramesFromAudioSamples)(samples) )
  long long AudioSamplesFromBytes(long long bytes) const AVS_BakedCode( return AVS_LinkCall(AudioSamplesFromBytes)(bytes) )
  long long BytesFromAudioSamples(long long samples) const AVS_BakedCode( return AVS_LinkCall(BytesFromAudioSamples)(samples) )
...
public:
  ...
  virtual void __stdcall GetAudio(void* buf, long long start, long long count, IScriptEnvironment* env) = 0;  // start and count are in samples
...
public:
  ...
  void __stdcall GetAudio(void* buf, long long start, long long count, IScriptEnvironment* env) { child->GetAudio(buf, start, count, env); }
...
public:
  ...
  void __stdcall GetAudio(void* buf, long long start, long long count, IScriptEnvironment* env);
